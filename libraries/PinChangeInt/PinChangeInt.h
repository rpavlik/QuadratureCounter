...
void PCintPort::attachInterrupt(uint8_t arduinoPin, PCIntvoidFuncPtr userFunc, int mode)
{
	...
	ports[portNum]->lastPinView = ports[portNum]->portInputReg;

	// map pin to PCIR register
	ports[portNum]->addPin(arduinoPin,mode,userFunc);
#ifdef DEBUG
...
void PCintPort::detachInterrupt(uint8_t arduinoPin)
{
	...
	ports[portNum]->delPin(digitalPinToBitMask(arduinoPin));
}
